...
public interface ActivityRepository extends JpaRepository<ActivityEntity, Long> {
    ...
    ActivityEntity getSelfActivityById(long activityId);
    ...
    @Query(value = "select a.* , (SELECT CASE WHEN COUNT(*) > 0 THEN true ELSE false END FROM personal_plan pp WHERE pp.activity_id = a.id AND pp.user_id = ?1) AS is_added_to_plan from activity a,information_detail ifmd where a.id = ifmd.activity_id and ifmd.user_id = ?1", nativeQuery = true)
